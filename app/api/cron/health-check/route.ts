...
async function checkServer(server: ServerRow): Promise<CheckResult> {
  ...
  try {
    ...
    // Count consecutive failures from recent checks (ordered by checkedAt DESC)
...
export async function GET(request: NextRequest) {
  ...
  for (let i = 0; i < allServers.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map((server) => checkServer(server))
    );
